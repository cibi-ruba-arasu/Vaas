...
const savedStatus = localStorage.getItem('guest_console_status_data')
...
watch(Console_Status, (newVal) => {
    localStorage.setItem('guest_console_status_data', JSON.stringify(newVal))
}, { deep: true })
...
const fetchConsole = async () => {
  ...
  try {
    ...
    if (guestGameIds.length === 0) {
      myGames.value = [];
      loading.value = false;
      return;
    }

    const fetchPromises = guestGameIds.map(id => 
...
<transition name="fade">
  ...
  <transition name="fade">
        <div v-if="showGuestAuthPopup" class="achievement-modal-overlay" @click="showGuestAuthPopup = false">
            <div class="achievement-modal-content" @click.stop style="max-width: 400px; padding: 40px;">
                ...
                <div class="ach-modal-actions" style="margin-top: 20px;">
                    ...
                    <button class="secondary-action-btn" @click="router.push('/register')">Create Account</button>
                </div>
            </div>
        </div>
    </transition>
