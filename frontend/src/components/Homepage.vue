...
const dropdownCategories = computed(() => {
  return ALL_CATEGORIES.filter(cat => {
    // Only hide mature tags if the user is underage
...
const toggleCategory = (catName) => {
  ...
  if (existingIdx !== -1) {
    deselectCategory(existingIdx); // Removes it if already selected
  } else {
  ...
  <div class="loom-container" :style="{ '--aura': auraColor, '--mouse-x': mouseX + 'px', '--mouse-y': mouseY + 'px' }">
    ...
    <div class="search-and-filter-band">
      <div class="search-bar-row">
        ...
        <div class="category-dropdown-wrapper" ref="categoryDropdown">
          <button 
            ...
            <div v-if="showCategories" class="cat-dropdown-menu glass-panel">
              <div class="cat-grid">
                <button 
                  ...
                  :key="cat.name" 
                  class="cat-chip"
                  :class="{ active: selectedCategories.some(c => c.name === cat.name) }"
      ...
      <TransitionGroup name="list" tag="div" class="active-filters" v-if="selectedCategories.length > 0">
        <span 
          ...
          :style="{ borderColor: cat.color, backgroundColor: `${cat.color}22` }"
        >
          <svg class="cat-icon" viewBox="0 0 24 24" fill="currentColor" style="width: 14px; height: 14px;">
            <path :d="cat.path" />
          </svg>
          {{ cat.name }} <span class="close-x">×</span>
        </span>
      </TransitionGroup>
    </div>
...
.close-x { font-weight: bold; line-height: 1; }
...
.list-enter-from, .list-leave-to { opacity: 0; transform: translateY(-10px) scale(0.9); }
...
.cat-dropdown-btn:hover {
  border-color: var(--aura) !important;
  color: #fff !important;
  box-shadow: 0 0 20px rgba(var(--aura), 0.3), 
              inset 0 0 15px rgba(var(--aura), 0.1);
  transform: translateY(-1px);
}

/* Special state when categories are active */
.cat-dropdown-btn.has-selections {
  background: rgba(var(--aura), 0.15) !important;
  border-color: var(--aura) !important;
  color: #fff !important;
...
.cat-dropdown-btn::after {
  ...
  width: 0;
  height: 1px;
  background: linear-gradient(90deg, transparent, var(--aura), transparent);
  transition: all 0.5s ease;
  transform: translateX(-50%);
}

.cat-dropdown-btn:hover::after, .cat-dropdown-btn.has-selections::after {
  width: 80%;
}

/* Pulse animation for the "Active" state */
@keyframes aura-pulse {
  0% { box-shadow: 0 0 10px var(--aura); }
  50% { box-shadow: 0 0 25px var(--aura), inset 0 0 10px var(--aura); }
  100% { box-shadow: 0 0 10px var(--aura); }
}

/* Subtle inner shine on the icon */
.chevron {
  filter: drop-shadow(0 0 2px var(--aura));
...
@media (max-width: 600px) {
  ...
  .search-bar-row {
    flex-direction: column; /* Stack search and button vertically */
    align-items: stretch;
    gap: 12px;
    width: 100%;
  }
  ...
  .active-filters {
    ...
    overflow-x: auto; /* Allow horizontal scroll if many filters are active */
    white-space: nowrap;
    padding-bottom: 5px;
  }
...
/* Subtle fix for the search results container on mobile */
@media (max-width: 600px) {
  .search-results {
    position: fixed;
    top: 20%;
...
/* Ensure Notifications and Settings containers sit above the search band */
.notification-wrapper, 
.settings-wrapper {
  position: relative;
...
.advanced-dropdown {
  ...
  /* Add a stronger shadow to help separate from the background glass */
  box-shadow: 0 20px 50px rgba(0, 0, 0, 0.9), 0 0 30px rgba(var(--aura), 0.1);
}

/* Ensure the search band stays below the nav dropdowns */
.search-and-filter-band {
  z-index: 100 !important; /* Lowered from previous relative height */
}
