...
const exportProject = async () => {
    try {
        ...
        // 2. Prompt the user to select the folder IMMEDIATELY after the click
        let dirHandle;
        try {
        ...
        // 3. NOW prompt for HTML file name
        let fileName = prompt(
        ...
        // 5. Create assets directory inside the chosen folder
        ...
        // 6. Find all unique media URLs across the project
        const urlMap = {}; 
        ...
        // 7. Download and save each media file
        ...
        // 8. Clone Canvas_Status and update URLs to point to local assets
        ...
        // 9. Package the game data (Safely escaping closing tags for embedding)
        const projectData = {
            nodes: exportStatus,
            globalVariables: globalVariables.value,
            rootNodeId: rootNodeId.value
        ...
        // 10. Save the HTML file with Embedded JSON, Workspace UI, and Inline Editing
...
<head>
    ...
    <style>
        ...
        /* 🚀 FIX: Made the entire left sidebar scrollable and preserved sizes */
        ...
        /* 🚀 FIX: Custom Scrollbar for Sidebar & Grid */
...
<body>
    ...
    <script>
        ...
        canvas.addEventListener('mouseup', (e) => { 
            isPanning = false; 
            if (!hasMoved) handleGraphClick(e.clientX, e.clientY);
        });
        ...
        canvas.addEventListener('touchmove', (e) => {
            // FIX 2: Only prevent default when actively panning/zooming the canvas to allow sidebar scrolling
        ...
        canvas.addEventListener('touchend', (e) => {
            ...
            if (e.touches.length === 0) {
                ...
                if (!hasMoved && e.changedTouches.length > 0) {
                    handleGraphClick(e.changedTouches[0].clientX, e.changedTouches[0].clientY);
                }
            }
        });
        ...
        function beginSceneExitForOption(compId, optionId) {
            ...
            setTimeout(() => {
                isSceneExiting = false;
                document.querySelectorAll('.weaver-input-wrapper').forEach(el => el.remove());
                if (nextNodeId !== null) loadNodeForGame(nextNodeId);
                else exitGame();
            }, maxExitTime);
        }
    ...
    <${'/'}script>
        ...
        triggerNotification("Project successfully exported to Workspace UI! 🚀", "success");
